 ...
 * Based on: time investment, voluntary engagement, commitment signals
 * @param {Object} task - Current task
 * @returns {number} Value score (0.0 to 1.0)
 */
function calculateValue(task) {
  // Factor 1: Time Investment (40% weight)
  ...
  // Factor 2: Voluntary Engagement (40% weight)
  ...
  // Factor 3: Commitment Signal (20% weight)
  ...
  const value =
    (0.4 * timeInvestment) +
    (0.4 * voluntaryEngagement) +
    (0.2 * commitmentSignal);
 ...
 * Based on: session interruptions, app switching, reminder behavior
 * @param {Object} task - Current task
...
function calculateImpulsiveness(task, userAvgImpulsiveness = 0.5) {
  ...
  // Factor 1: Session Interruptions (40% weight)
  ...
  // Factor 2: App Switching (40% weight)
  const appBackgroundEvents = task.appBackgroundEvents || 0;
  const totalSessionMinutes = task.actualTimeSpent || 1;
  const appSwitching = Math.min(appBackgroundEvents / totalSessionMinutes, 1.0);

  // Factor 3: Reminder Behavior (20% weight)
